...
import {latest_installer_URL, one_line_powershell} from "@site/static/config.js"
...
1. WinUSB を対象の USB デバイスに適用するには、デバイスのベンダー ID を知る必要があります。タスクバーの検索ボックスに「device manager」と入力し、メニューから**Device Manager**を選択して、U3Vカメラデバイスを確認してください。
...
U3V カメラデバイスをダブルクリックし、ポップアップウィンドウの**詳細**タブを選択します。ドロップダウンメニューから**ハードウェア ID**を選択し、`VID_`の後にベンダー ID を見ることができます。下記の例では、それが`2676`です。
