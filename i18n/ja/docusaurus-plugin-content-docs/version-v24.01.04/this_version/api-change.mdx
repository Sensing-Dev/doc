...
古いAPIはまだv24.01.04でサポートされていることに注意してください。
...
Sensing-Dev v24.01.04（ion-kit 1.6.0）では、USB 3 Vision Cameraを使用するBBはこれ以上この入力を必要とせず、`builder` のインスタンスが解放されるとカメラが停止します。

インスタンスが解放されるタイミングを見たい場合は、環境変数`ION_LOG_LEVEL`を`debug`に設定してください。詳しくは[デバッグのヒント](../lessons/ion-log)を参照してください。
...
Sensing-Dev v23.11.01（ion-kit 0.3.5）では、各入力/出力 `Port` に入力値を設定するために `PortMap` のオブジェクトが必要でした。
