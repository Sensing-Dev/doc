...
DLLライブラリがあるディレクトリを`PATH`環境変数に追加するのを忘れないでください。
:::
...
ページのトップにあるインストーラスクリプトをダウンロードするか、<a href={this_version.latest_installer_URL}>こちら</a>からダウンロードしてください。

<AddMsiInstructionIfLatest islatest={this_version.is_latest} version={this_version.latest_version}/>
