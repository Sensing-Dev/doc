...
3. **windows**:  
    Install the C++ version of sensing-dev. make sure GST_PLUGIN_PATH is set.
3. **linux**:  
   Set the GST_PLUGIN_PATH environment variable and sensing-dev library_path:
   ```bash
...
<GenerateTutorialLink language="" tag={tutorial_version} tutorialfile="gstreamer/tutorial1_display" />